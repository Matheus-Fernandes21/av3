...
public class tela3 extends JFrame {
	...
	public tela3() {
		...
		private void consulta() {
			String sql = "select * from dbo.aluguel where cod_carro=?";
		try {
			...
			txtrua.setText(rs.getString(4));
				...
				JOptionPane.showMessageDialog(null,"data nao especificada");
		...
		private void adicionar(){
			String sql = "insert into dbo.aluguel(cod_carro,nome_cliente,data_aluguel,data_devolucao)values(?,?,?,?)";
		try {
			pst=conexao.prepareStatement(sql);
			pst.setString(1,txtcpf.getText());
		...
		private void alterar(){
			String sql="update dbo.aluguel set nome_cliente=?,data_aluguel=?,data_devolucao=? where cod_carro=?";
		...
		int confirma=JOptionPane.showConfirmDialog(null, "Deseja mesmo excluir os dados","atencao",JOptionPane.YES_NO_OPTION);
		if(confirma==JOptionPane.YES_OPTION) {
		String sql="delete from dbo.aluguel where cod_carro=?";
		try {
		pst=conexao.prepareStatement(sql);
		pst.setString(1, txtcpf.getText());
		...
		JLabel lblNewLabel_5 = new JLabel("Nome do cliente");
		...
		JLabel lblendereco = new JLabel("Data de devolucao");
		lblendereco.setBounds(10, 370, 126, 14);
		...
		txtcodcarro = new JLabel("Codigo do carro");
		txtcodcarro.setBounds(218, 338, 87, 14);
		contentPane.add(txtcodcarro);
		
		lblNewLabel_9 = new JLabel("Data de aluguel");
		lblNewLabel_9.setBounds(218, 370, 116, 14);
		...
		JButton Consultarbd = new JButton("Consultar ");
